...
twist_2D = np.zeros(2, np.float32)

#driver output mask
output_mask = driver_constant*np.array([-1.0, 1.0], np.float32)
...
disable_output=(np.array([0,0], np.int16)).tobytes()
...
def Callback(data):
    ...
    twist_2D = np.array((data.twist.linear.x, data.twist.angular.z), np.float32)
...
rospy.Subscriber("cmd_vel_udp", UDPmessage, Callback)
# ros set rate
r = rospy.Rate(100)
